...
num_training = 100000
learning_rate = 0.1
model_save_path = './model/RESNET-NEW/'
brestore = False
restore_iter = 1000
batch_size = 16
...
model = ftn.CNN_RESNET_NEW_Light()
...
for it in tqdm(range(num_training + 1), ncols=120):
    ...
    # 추가기능 : 1000 step 마다 최고의 모델을 찾아서 로딩한 이후, 그 모델을 불러온 후 학습을 재시작
    # array에 acc를 저장해두고, 1000step 마다 acc를 비교하여 최고 acc 모델을 저장. ( 100setp 단위)
    # array 형태를 (iteration, acc, model_path) 로 저장 후 삭제하기 용이하게.
    # -> 너무 잦은 불러오기로 학습이 너무 오래걸림 -> 1000스텝마다 저장.
    ...
    if it % 10000 == 0:
        ...
        if it == 10000:
            ...
            if model_ckpt[-1][1] > model_ckpt_hof[-1][1]:
                ...
                lr_count += 1
                if lr_count >= 1: # 2번 연속으로 lr 감소가 없는 경우, lr 감소 # 처음 5로 설정했을때, 너무 오랜시간을 학습하는 것으로 보여, 2회로 줄임.
                    # 그래도 오래 걸리는 것 같아서 1로 줄임.
